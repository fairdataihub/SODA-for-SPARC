...
const getPennsieveAgentPath = () => {
  if (process.platform === "win32" || process.platform === "cygwin") {
    ...
    throw new Error(`Cannot find pennsieve path`);
  } else {
...
const startPennsieveAgent = async (pathToPennsieveAgent) => {
  console.log("startPennsieveAgent");
  return new Promise((resolve, reject) => {
...
const startPennsieveAgentAndGetVersion = async () => {
  ...
  try {
    ...
    throw new Error("Unable to retrieve the latest version of the Pennsieve agent from GitHub");
  }
  ...
  try {
    ...
    throw new Error("Unable to decect Pennsieve agent installation");
  }
  ...
  try {
    ...
    throw new Error("Unable to start Pennsieve agent");
  }
  ...
  try {
    ...
    console.log("Error getting Pennsieve agent version: ", error);
  }
  console.log(pennsieveAgentVersion);
  console.log(latest_agent_version);
  console.log(pennsieveAgentVersion === latest_agent_version);
  if (pennsieveAgentVersion !== latest_agent_version) {
    ...
    if (result) {
      shell.openExternal(browser_download_url);
      shell.openExternal("https://docs.pennsieve.io/docs/uploading-files-programmatically");
      throw new Error("Pennsieve agent is not up to date");
    }
  }
};
...
const run_pre_flight_checks = async (check_update = true) => {
  ...
  try {
    ...
    await startPennsieveAgentAndGetVersion();
    ...
    log.error("Unable to start the Pennsieve Agent.");
    log.error(error);
    console.log(error);
    return false;
  }
