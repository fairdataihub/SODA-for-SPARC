...
4. Each file/folder has a subkey "type" which is a string with value either "virtual" (only for folders, if created in SODA), "local", or "ps"
...
6. Each non virtual file/folder has a subkey "action" which is an array containing "existing" (if it from an existing local or ps dataset) or "new" (if imported).
...
5. Each file has a subkey "destination" which is a string with value either "generate-dataset" (meaning it is part of the curate process and destination info is available in the "generate-dataset" main key), "local" (generate the file locally), or "ps" (generate the dataset on ps, look for the ps account and dataset keys for info)
...
1. Has a key "destination" which is a string with value either "generate-dataset" (meaning it is part of the curate process and destination info is available in the "generate-dataset" main key), "local" (generate the file locally), or "ps" (generate the dataset on ps, look for the ps account and dataset keys for info)
...
1. Has a key "destination" which is a string with value either "local" (generate the file locally) or "ps" (generate the dataset on ps, look for the ps account and dataset keys for info)
...
1. Adding new ps or Airtable accounts will be done outside from the json structure (i.e., user enter API key, we add account)
