...
def get_role(dataset):
    ...
    try:
        r = requests.get(f"{PENNSIEVE_URL}/datasets/{selected_dataset_id}/role", headers=create_request_headers(get_access_token()), verify=path_to_cert)
        r.raise_for_status()
        role = r.json()["role"]
        return {"role": role}
...
def get_dataset_by_id(dataset_name_or_id):
    ...
    r = requests.put(f"{PENNSIEVE_URL}/datasets/{selected_dataset_id}", headers=headers, verify=path_to_cert)
...
def get_current_collection_names(account, dataset):
    ...
    r = requests.get(f"{PENNSIEVE_URL}/datasets/{selected_dataset_id}/collections", headers=create_request_headers(get_access_token()), verify=path_to_cert)
    r.raise_for_status()
...
def upload_collection_names(account, dataset, tags):
    ...
    for tag in tags:
        ...
        r = requests.put(f"{PENNSIEVE_URL}/datasets/{selected_dataset_id}/collections", json=jsonfile, headers=create_request_headers(get_access_token()), verify=path_to_cert)
        r.raise_for_status()
        result = r.json()
...
def remove_collection_names(account, dataset, tags):
    ...
    for tagid in tags:
        r = requests.delete(f"{PENNSIEVE_URL}/datasets/{str(selected_dataset_id)}/collections/{str(tagid)}", headers=create_request_headers(get_access_token()), verify=path_to_cert)
        r.raise_for_status()
...
def reserve_dataset_doi(dataset):  # sourcery skip: extract-method
    ...
    try:
        doi_request = requests.post(f"{PENNSIEVE_URL}/datasets/{dataset_id}/doi", headers=create_request_headers(get_access_token()), verify=path_to_cert)
...
def get_dataset_doi(dataset):
    ...
    dataset_id = get_dataset_id(dataset)
    try:
        doi_request = requests.get(f"{PENNSIEVE_URL}/datasets/{dataset_id}/doi", headers=create_request_headers( get_access_token()), verify=path_to_cert)
...
def get_package_type_counts(dataset_name):
    ...
    r = requests.get(f"https://api.pennsieve.io/datasets/{dataset_id}/packageTypeCounts", headers=create_request_headers(get_access_token()), verify=path_to_cert)
    r.raise_for_status()
