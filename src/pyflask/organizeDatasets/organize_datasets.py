...
from utils import (
    ...
    connect_pennsieve_client, get_dataset_id,
    PennsieveActionNoPermission, PennsieveDatasetCannotBeFound,
    EmptyDatasetError, LocalDatasetMissingSpecifiedFiles,
    PennsieveUploadException, create_request_headers, get_users_dataset_list,
    PennsieveDatasetNameInvalid, PennsieveDatasetNameTaken, TZLOCAL, GenerateOptionsNotSet,
    PennsieveDatasetFilesInvalid, get_upload_manifests
)
from pysodaUtils import check_forbidden_characters_ps
...
logger = get_namespace_logger(NamespaceEnum.CURATE_DATASETS)
...
def ps_update_existing_dataset(soda, ds, ps, resume):
    ...
    #     soda.copy()
    # )["soda_object"]
    current_bf_dataset_files_folders = []
...
def ps_upload_to_dataset(soda, ps, ds, resume=False):
    ...
    try:
        ...
        logger.info(f"{starting_point} and generation option {generate_option}")
        ...
        if generate_option == "new" and starting_point == "new":
            ...
            if resume == False or resume == True and not vs:
                ...
                logger.info(f"List of upload files {list_upload_files}")
