...
ipcMain.on("warning-delete-award", (event) => {
  const options = {
    ...
    title: "Warning",
    message:
      "This will delete all presaved information regarding the award number, are you sure you want to continue?",
    buttons: ["Yes", "No"],
  };
...
ipcMain.on("warning-clear-table", (event) => {
  const options = {
    ...
    title: "Warning",
    message:
      "This will erase all your progress, are you sure you want to continue?",
    buttons: ["Yes", "No"],
  };
  dialog.showMessageBox(BrowserWindow.getFocusedWindow(), options, (index) => {
...
ipcMain.on(
  "warning-new-version",
  (event, currentAppVersion, latestAppVersion) => {
    const options = {
      type: "info",
      ...
      message:
        "You have version " +
        currentAppVersion +
        " of SODA while the latest version is " +
        latestAppVersion +
        ". We suggest to uninstall the current version and download the latest version to make sure you are up-to-date with the SPARC curation rules and have access to the latest features of SODA!",
    };
