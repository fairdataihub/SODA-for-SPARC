...
class DictValidator:
    ...
    def check_high_level_metadata_files(self, jsonStruct):
        ...
        if len(allFiles) == 0:
            ...
            for c in allFiles:
                ...
                if cname == self.reqMetadataFileNames[0]:  # submission file
                    if (
                        extension in self.metadataFileFormats1
                    ...
                    fullFileName
                    == self.reqMetadataFileNames[3] + self.metadataFileFormats2[0]
                ):  # README.txt file
                    if getsize(filePath) > 0:
                        rm = 1
                    else:
                    ...
                    fullFileName
                    == self.optMetadataFileNames[1] + self.metadataFileFormats2[0]
                ):  # CHANGES.txt file
                    chg = 1
        ...
        check4 = (
            "A 'subjects' metadata file is included in either xlsx, csv, or json format"
        )
        ...
        check5 = (
            "A 'samples' metadata file is included in either xlsx, csv, or json format"
        )
    ...
    def check_manifest_file_included(self, jsonStruct):
        ...
        check1c1 = (
            "It is likely you chose the 'all folders with at least one file' option"
        )
        check1c2 = (
            "It is likely you chose the 'all high-level SPARC folders only' option"
        )
    ...
    def check_submission_file(self, submFilePath):
        ...
        expectedSubmissionFullName = [
            (submissionName + i) for i in self.metadataFileFormats1
        ]
        ...
        if os.path.isfile(submFilePath):
            ...
            if extension == ".csv":
                ...
                self.warnings.append(
                    "The SODA validator currently doesn't support the json format so your file cannot be validated. This will be implemented in a future release."
                )
                return
            ...
            for header in fileHeaders:
                if header not in (
                    self.reqSubmissionHeaders + self.optSubmissionHeaders
                ):
                    nonstandardh = 1
        ...
        check1f = (
            "The first header (in cell A1) MUST be " + self.reqSubmissionHeaders[0]
        )
        ...
        check1f4 = (
            "Only the following headers are expected: "
            + str(self.reqSubmissionHeaders[0])
            + str(self.reqSubmissionHeaders[1])
            + ","
            + str(self.optSubmissionHeaders[0])
    ...
    def check_dataset_description_file(self, ddFilePath):
        ...
        if not os.path.isfile(ddFilePath):
            ...
            if extension == ".csv":
                ...
                if UTF8status == 0:
                    ...
                    return
                df = pd.read_csv(
                    ddFilePath, header=None
                )  # headers ignored here since pandas modify duplicat headers
            elif extension == ".xlsx":
            ...
            if fileHeaders[0] != self.reqddHeaders[0]:
                ...
                for item in removeHeadersList:
                    if item in fileHeadersVal:
                        fileHeadersVal.remove(item)
                ...
                if columnfail == 0 and headersfail == 0:
                    ...
                    if len(index_empty) > 0:
                        ...
                        for i in index_empty:
                            valuemandemptyList += (
                                " " + dfMand[self.reqddHeaders[0]].iloc[i] + ","
                            )

                    # check for non terms for "Value" for mandatory fields
                    index_none = [
                        i for i, e in enumerate(valueColMod) if e in self.noneTerms
                    ]
                    if len(index_none) > 0:
                        valuemandnoneterm = 1
                        for i in index_none:
                    ...
                    if (
                        valuemandempty == 0
                        and valuemandnoneterm == 0
                        and valueoptnoneterm == 0
                    ):
                        valuefail = 0
                    else:
                        valuefail = 1
                    ...
                    if valuefail == 0:
                        ...
                        keywordsList = (
                            dfv.loc[dfv[metadataEl] == selectedEl].iloc[0].values
                        )
                        keywordsList = np.delete(
                            keywordsList, np.argwhere(keywordsList == selectedEl)
                        )
                        keywordsList = np.delete(
                            keywordsList, np.argwhere(keywordsList == self.empty)
                        )
                        ...
                        contributorsList = np.delete(
                            contributorsList,
                            np.argwhere(contributorsList == selectedEl),
                        )
                        contributorsList = np.delete(
                            contributorsList,
                            np.argwhere(contributorsList == self.empty),
                        )
                        ...
                        if (
                            contributorsnameunqfail == 0
                            and contributorsnameformatfail == 0
                        ):
                            ...
                            nameList = (
                                dfv.loc[dfv[metadataEl] == selectedEl1].iloc[0].values
                            )
                            nameList = np.delete(
                                nameList, np.argwhere(nameList == selectedEl1)
                            )
                            orcidList = (
                                dfv.loc[dfv[metadataEl] == selectedEl2].iloc[0].values
                            )
                            orcidList = np.delete(
                                orcidList, np.argwhere(orcidList == selectedEl2)
                            )
                            affiliationList = (
                                dfv.loc[dfv[metadataEl] == selectedEl3].iloc[0].values
                            )
                            affiliationList = np.delete(
                                affiliationList,
                                np.argwhere(affiliationList == selectedEl3),
                            )
                            roleList = (
                                dfv.loc[dfv[metadataEl] == selectedEl4].iloc[0].values
                            )
                            roleList = np.delete(
                                roleList, np.argwhere(roleList == selectedEl4)
                            )
                            contactpersonList = (
                                dfv.loc[dfv[metadataEl] == selectedEl5].iloc[0].values
                            )
                            contactpersonList = np.delete(
                                contactpersonList,
                                np.argwhere(contactpersonList == selectedEl5),
                            )

                            for name, orcid, affiliation, role, contactperson in zip(
                                nameList,
                                orcidList,
                                affiliationList,
                                roleList,
                                ...
                                if name != self.empty:
                                    if (
                                        orcid == self.empty
                                        or affiliation == self.empty
                                        or role == self.empty
                                        or contactperson == self.empty
                                    ):
                        ...
                        fundingList = np.delete(
                            fundingList, np.argwhere(fundingList == selectedEl)
                        )
                        fundingList = np.delete(
                            fundingList, np.argwhere(fundingList == self.empty)
                        )
                        ...
                        articleList = np.delete(
                            articleList, np.argwhere(articleList == selectedEl1)
                        )
                        ...
                        protocolList = np.delete(
                            protocolList, np.argwhere(protocolList == selectedEl2)
                        )
                        for protocol in protocolList:
                            if protocol != "empty":
                                if (
                                    "https://doi.org/" not in protocol
                                    or "https://www.protocols.io/" not in protocol
                                ):
                        ...
                        try:
                            ...
                            print(
                                f"Number of subjects: {numSubjects} is not a valid integer."
                            )
                        ...
                        try:
                            ...
                            print(
                                f"Number of samples: {numSamples} is not a valid integer."
                            )
                        ...
                        if (
                            completeness not in self.completnessInfo
                            and completeness != self.empty
                        ):
                        ...
                        if parentIDList != self.empty:
                            if "," in parentIDList:
                                parentIDList = [
                                    parentID for parentID in parentIDList.split(",")
                                ]
                                parentIDList = [
                                    parentID.strip() for parentID in parentIDList
                                ]
        ...
        check1_h4 = (
            "All headers must be unique. The following header(s) is/are duplicated: "
        )
        ...
        check_contributorsname = (
            "Contributors names are unique and in the format 'Last, First Middle'"
        )
        check_contributorsname_f1 = (
            "The following contributor name(s) may have been included more than once: "
        )
        ...
        check_contributorinfo_f = (
            "Check that above requirements are met for the following 'Contributors': "
        )
        ...
        check_contactperson_f1 = (
            "The following 'Is Contact Person' element is not 'Yes' or 'No': "
        )
        ...
        check_fundingsource_f = (
            "Make sure the following refers to a single funding source: "
        )
        ...
        check_links_f = (
            "Make sure the above condition is met in the following column(s): "
        )

        check_articles = (
            "'Originating Article DOI' is in the format https://doi.org/xxxx"
        )
        check_articles_f = (
            "Make sure the above condition is for the following DOI article(s): "
        )
                ...
                if valuefail == 0:
                    ...
                    if contributorsnameunqfail == 0 and contributorsnameformatfail == 0:
                        ...
                        if contributorsnameunqfail == 1:
                            msg += (
                                "--"
                                + check_contributorsname_f1
                                + contributorsnameunqfailList[:-1]
                            )

                        if contributorsnameformatfail == 1:
                            msg += (
                                "--"
                                + check_contributorsname_f2
                                + contributorsnameformatfailList[:-1]
                            )
                    ...
                    if parentidvaluefail == 0 and parentidformatfail == 0:
                        ...
                        if parentidformatfail == 1:
                            msg += (
                                "--" + check_parentID_f2 + parentidformatfailList[:-1]
                            )
                        self.fatal.append(msg)
...
def cleanDataFrame(df):
    ...
    for header in list(df)[::-1]:
        if (
            "Empty." in df[header].iloc[0]
            and len(set(df[header].values[1:])) == 1
            and df[header].iloc[1] == "empty"
        ):
            df = df.drop(header, 1)
        else:
            break
    ...
    for rownum in df.index.values[::-1]:
        if (
            len(set(df.iloc[rownum].values)) == 1
            and df[list(df)[0]].iloc[rownum] == "empty"
        ):
            df = df.drop(rownum)
        else:
            break
    ...
    for i in range(len(contentDataset)):
        ...
        if os.path.isdir(contentPath):
            ...
            for j in range(len(filesInFolder)):
                ...
                listPathFilesinFolder.append(
                    os.path.join(contentPath, fileNameInFolder)
                )
...
def validate_high_level_metadata_files(jsonStruct):
    ...
    (
        ...
        isSubjects,
        isSamples,
    ) = validator.check_high_level_metadata_files(jsonStruct)
