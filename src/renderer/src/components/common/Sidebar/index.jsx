...
const Sidebar = ({ id }) => {
  ...
  // Only wrap in nav if id is guided-sidebar
  if (id === "guided-sidebar") {
    return <nav className={classes.navbar}>{sidebarContent}</nav>;
  }

  // For main-sidebar, return div without nav wrapper
  return <div className={classes.navbar}>{sidebarContent}</div>;
};
