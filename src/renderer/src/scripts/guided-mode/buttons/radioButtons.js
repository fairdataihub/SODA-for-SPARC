...
export const updateGuidedRadioButtonsFromJSON = (parentPageID) => {
  ...
  for (const reactCard of reactComponentCards) {
    ...
    if (!buttonId) {
      window.log?.error?.(
        `[updateGuidedRadioButtonsFromJSON] data-checkbox-card-id missing for React element: ${JSON.stringify(
          reactCard
        )}`
      );
      continue;
    ...
    if (!checkboxData) {
      window.log?.error?.(
        `[updateGuidedRadioButtonsFromJSON] No checkboxData found for React buttonId: ${buttonId}`
      );
      continue;
    }
