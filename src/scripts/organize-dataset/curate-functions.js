...
const checkAvailableSpace = () => {
  ...
  //calculating for auto generated manifest file
  //at most it should not exceed 1MB
  var autoGeneratedManifest = 1048576;
  var location = document.getElementById("input-destination-generate-dataset-locally").getAttribute("placeholder");
    ...
    client.invoke(
      ...
      (error, res) => {
        if(error) {
          ...
          var warningText = "Please free up " + roundToHundredth((folderSizeMB)) + "MB " + 
          ...
          if(folderSizeMB > 1000) {
            ...
            warningText = "Please free up " + roundToHundredth((folderSizeMB)) + "GB " + 
              "or consider uploading directly to Pennsieve."
            //if bigger than a tb then convert to that
            if(folderSizeMB > 1000) {
              folderSizeMB = roundToHundredth(folderSizeMB/(1024));
              freeMemoryMB = roundToHundredth(freeMemoryMB/(1024));
              warningText = "Please free up " + roundToHundredth((folderSizeMB)) + "TB " + 
                "or consider uploading directly to Pennsieve."
            }
          }
